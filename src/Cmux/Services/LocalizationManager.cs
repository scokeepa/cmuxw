...
public sealed class LocalizationManager : INotifyPropertyChanged
{
    ...
    private readonly Dictionary<string, Dictionary<string, string>> _translations = new(Cmp)
    {
        ["ko"] = new Dictionary<string, string>(Cmp)
        {
            ...
            ["Close this side tab?"] = "이 사이드 탭을 닫을까요?",
            ["Close workspace '{0}'?"] = "워크스페이스 '{0}'을(를) 닫을까요?",
            ["Reset this pane session?"] = "이 패널 세션을 초기화할까요?",
            ["Auto Copy"] = "자동 복사",
            ...
            ["Browser Default URL"] = "브라우저 기본 URL",
            ["Default URL opened by the browser button/menu/shortcut."] = "브라우저 버튼/메뉴/단축키로 열리는 기본 URL입니다.",
            ...
            ,["Layout shrink will close N panes"] = "이 레이아웃으로 바꾸면 패널 {0}개가 닫히고 해당 터미널·브라우저 세션이 종료됩니다."
            ,["Layout shrink active sessions hint"] = "닫히는 패널 중 일부에 실행 중인 셸, 데몬 세션, 브라우저 또는 명령 기록이 있습니다."
            ,["Irregular split layout reset for toolbar layout"] = "현재 분할이 균일한 그리드가 아닙니다. 이 레이아웃을 적용하려면 포커스된 패널만 남기고 나머지를 닫아야 합니다. 계속할까요?"
            ,["Main stack layout will close N panes"] = "메인+스택 레이아웃을 만들려면 패널을 3개로 맞춰야 합니다. 지금은 패널 {0}개가 닫힙니다(포커스 패널은 유지)."
            ...
            ,["Close pane"] = "패널 닫기"
            ,["Reset pane session"] = "이 패널 세션만 초기화"
            ,["Reset sidebar view"] = "사이드바 보기 초기화(필터·읽음)"
            ,["Clear visible agent thread"] = "보이는 에이전트 스레드만 비우기"
            ,["Select a thread to clear."] = "비울 스레드를 먼저 선택하세요."
            ,["Clear all session data"] = "저장된 모든 세션 데이터 삭제"
            ,["This permanently deletes session.json (window layout) and all agent conversation threads. The app will restart. Continue?"] = "session.json(창 레이아웃)과 모든 에이전트 대화 스레드가 영구 삭제됩니다. 앱이 다시 시작됩니다. 계속할까요?"
            ,["Session data was cleared. Restarting."] = "세션 데이터가 삭제되었습니다. 다시 시작합니다."
            ,["Clear all session data detail"] = "저장된 창 레이아웃(session.json)과 모든 에이전트 대화 스레드를 삭제합니다. 완료 후 앱이 다시 시작됩니다."
            ,["Erase all session history"] = "모든 세션 기록 삭제"
            ,["Removes saved window layout (session.json) and all agent conversation threads. The app restarts after clearing."] = "저장된 창 레이아웃(session.json)과 모든 에이전트 대화 스레드를 삭제합니다. 완료 후 앱이 다시 시작됩니다."
            ,["Close Pane"] = "패널 닫기"
        ...
        ["zh-CN"] = new Dictionary<string, string>(Cmp)
        {
            ...
            ["Close this pane?"] = "确定关闭此面板吗？",
            ["Close this side tab?"] = "确定关闭此侧边标签吗？",
            ["Close workspace '{0}'?"] = "确定关闭工作区“{0}”吗？",
            ["Reset this pane session?"] = "确定重置此面板会话吗？",
            ["Auto Copy"] = "自动复制",
            ["Copy to clipboard on text selection"] = "选择文本时复制到剪贴板",
            ["Ctrl+Click URLs"] = "Ctrl+点击 URL",
            ["Open URLs with Ctrl+Click"] = "Ctrl+点击打开 URL",
            ["Browser Default URL"] = "浏览器默认 URL",
            ["Default URL opened by the browser button/menu/shortcut."] = "通过浏览器按钮/菜单/快捷键打开的默认 URL。",
            ...
            ["Change layout"] = "更改布局",
            ["Layout shrink will close N panes"] = "切换到此布局将关闭 {0} 个面板，其终端或浏览器会话将结束。",
            ["Layout shrink active sessions hint"] = "其中部分面板有正在运行的 Shell、守护进程会话、浏览器或命令历史。",
            ["Irregular split layout reset for toolbar layout"] = "当前分屏不是完整网格。要应用此布局，需要先关闭除焦点面板外的所有面板。是否继续？",
            ["Main stack layout will close N panes"] = "主面板+堆叠需要 3 个面板。当前将关闭 {0} 个面板（保留焦点面板）。",
            ...
            ["Close pane"] = "关闭面板",
            ["Reset pane session"] = "仅重置此面板会话",
            ["Reset sidebar view"] = "重置侧栏视图（筛选与已读）",
            ["Clear visible agent thread"] = "仅清空当前可见的智能体线程",
            ["Select a thread to clear."] = "请先选择要清空的线程。",
            ["Clear all session data"] = "清除所有已保存的会话数据",
            ["This permanently deletes session.json (window layout) and all agent conversation threads. The app will restart. Continue?"] = "将永久删除 session.json（窗口布局）以及所有智能体会话线程。应用将重启。是否继续？",
            ["Session data was cleared. Restarting."] = "会话数据已清除。正在重启。",
            ["Clear all session data detail"] = "删除已保存的窗口布局（session.json）以及所有智能体会话线程，完成后将重启应用。",
            ["Erase all session history"] = "清除所有会话记录",
            ["Removes saved window layout (session.json) and all agent conversation threads. The app restarts after clearing."] = "将删除已保存的窗口布局（session.json）以及所有智能体会话线程，完成后应用会重启。",
            ["Close Pane"] = "关闭面板",
            ["Clear Terminal"] = "清空终端",
            ["Select All"] = "全选",
            ["Paste"] = "粘贴",
