...
public static class TerminalThemes
{
    ...
    public static Dictionary<string, TerminalTheme> BuiltIn { get; } = new()
    {
        ...
        ["Default Light"] = new TerminalTheme
        {
            ...
            Foreground = new TerminalColor(0x0C, 0x0C, 0x0C),
            CursorColor = new TerminalColor(0x00, 0x00, 0x00),
            SelectionBg = new TerminalColor(0x26, 0x7A, 0xDB),
            Palette =
            [
                // VS Code / Cursor light terminal palette — maximum contrast on white
                new(0x00, 0x00, 0x00), // 0  black
                new(0xCD, 0x31, 0x31), // 1  red
                new(0x00, 0x7A, 0x2C), // 2  green
                new(0x79, 0x56, 0x00), // 3  yellow  (dark gold — readable as fg on white)
                new(0x00, 0x66, 0xBF), // 4  blue
                new(0xAF, 0x00, 0xDB), // 5  magenta
                new(0x11, 0xA8, 0xCD), // 6  cyan
                new(0x3A, 0x3A, 0x3A), // 7  white  (dark grey on white bg)
                new(0x66, 0x66, 0x66), // 8  bright black (grey)
                new(0xCD, 0x31, 0x31), // 9  bright red
                new(0x14, 0xCE, 0x14), // 10 bright green
                new(0x80, 0x6B, 0x00), // 11 bright yellow (dark gold — readable on white bg)
                new(0x3B, 0x8E, 0xEA), // 12 bright blue
                new(0xBC, 0x3F, 0xBC), // 13 bright magenta
                new(0x29, 0xB8, 0xDB), // 14 bright cyan
                new(0x0C, 0x0C, 0x0C), // 15 bright white (near-black on white bg)
            ],
        },
